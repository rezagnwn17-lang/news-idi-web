...
latest_entries = []
user_news_data = {} # Memori untuk menyimpan draf berita manual

# ==========================================
# FUNGSI INTI: MENGUNGGAH KE GITHUB
# ==========================================
def eksekusi_publish_github(message, judul, link_sumber, gambar_url, tanggal, ringkasan):
    try:
        ...
        # Script HTML agar jika diklik, otomatis redirect ke link sumber asli
...
def sambutan(message):
    ...
    teks += "👉 /buat - Tulis dan publish berita buatan Anda sendiri!\n"
...
def proses_judul_manual(message):
    ...
    msg = bot.reply_to(message, "✨ Bagus! Langkah 2: Ketik **RINGKASAN / KONTEN SINGKAT** dari berita tersebut (1-2 kalimat):")
...
def proses_ringkasan_manual(message):
    ...
    msg = bot.reply_to(message, "🔗 Terakhir! Langkah 3: Masukkan **LINK TUJUAN** (Misal: link pendaftaran, PDF, atau website lain). \n\n*(Ketik tanda - (strip) jika tidak ada link khusus)*:")
...
def proses_publish_manual(message):
    ...
    if link == '-' or link == '':
        link = "#" # Jika tidak ada link, tombolnya tidak mengarah kemana-mana
    
    judul = user_news_data[chat_id]['judul']
    ringkasan = user_news_data[chat_id]['ringkasan']
    link_sumber = link
    tanggal = datetime.now().strftime("%d %B %Y")
    # Gambar default bertema dokter/kesehatan untuk berita manual
    gambar_url = "https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=600&q=80" 
    
    bot.reply_to(message, "🚀 Sedang merakit dan menerbitkan berita buatan Anda ke website... ⏳")
    eksekusi_publish_github(message, judul, link_sumber, gambar_url, tanggal, ringkasan)
